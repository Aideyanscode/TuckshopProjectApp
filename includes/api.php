...
function json_response(array $data, int $code = 200): void
{
    ...
    header('Access-Control-Allow-Headers: Content-Type, X-Admin-Token, X-Seller-Token, X-Parent-Token, X-Student-Token');
...
function get_parent_from_request(): ?array
{
    static $parent = null;
    if ($parent !== null) {
        return $parent ?: null;
    }
    $token = trim($_SERVER['HTTP_X_PARENT_TOKEN'] ?? '');
    if ($token === '') {
        $parent = false;
        return null;
    }
    require_once __DIR__ . '/database.php';
    $stmt = db()->prepare('SELECT id, email, full_name FROM parents WHERE api_token = ? AND is_active = 1');
    $stmt->execute([$token]);
    $row = $stmt->fetch();
    $parent = $row ?: false;
    return $row ?: null;
...
{
    if (!get_parent_from_request()) {
        json_error('Parent login required', 401);
    }
}

function get_student_from_request(): ?array
{
    static $student = null;
    if ($student !== null) {
        return $student ?: null;
    }
    $token = trim($_SERVER['HTTP_X_STUDENT_TOKEN'] ?? '');
    if ($token === '') {
        $student = false;
        return null;
    }
...
{
    if (!get_student_from_request()) {
        json_error('Student login required', 401);
    }
}
